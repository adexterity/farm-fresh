<script setup>

</script>

<template>
    <section class="border min-h-[100vh] py-[35px] font-body relative">
        <img class="absolute bottom-0 left-0" src="../assets/why-watermelon-icon.svg" />

        <div class="container flex justify-between items-center gap-4 px-[10px]">
            <div class="relative max-w-[full]">
                <img class="rounded-[25px] max-w-[full] h-auto" src="../assets/woman-eating.png"
                    alt="picture of a lady eating" />
            </div>
            <div class="flex flex-col items-center">
                <h3 class="font-bold text-[40px] mb-[30px] text-black">Why Choose Farm Fresh?</h3>

                <img src="../assets/why-vehicle-icon.svg" />
                <h4 class="font-bold py-4 text-black">Nationwide Delivery</h4>
                <p class="mb-5 text-[24px] text-center">We have outlets located near you. you'll get your order as soon as
                    possible</p>


                <img src="../assets/why-checkMark-icon.svg" />
                <h4 class="font-bold py-4 text-black">Authorized Natural Sources</h4>
                <p class="mb-5 text-[24px] text-center">Our products are strictly quality controlled and independently
                    verified</p>


                <img src="../assets/why-nutri-icon.svg" />
                <h4 class="font-bold py-4 text-black">High Nutrient </h4>
                <p class="mb-5 text-[24px] text-center">We have outlets located near you. you'll get your order as soon as
                    possible</p>

            </div>
        </div>
    </section>
</template>